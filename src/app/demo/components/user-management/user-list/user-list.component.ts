...
export class UserListComponent implements OnInit {
    ...
    onSubmit() {
        let request: Observable<any> = this.editMode
            ? this.userControllerService.updateUserById({
                  id: this.doctorId,
                  body: { ...this.doctorForm.value },
              })
            : this.userControllerService.signUp({
                  body: this.doctorForm.value,
              });
    ...
    exportDoctorEmails() {
        this.userControllerService.exportDoctorsEmails().subscribe({
            next: (emails) => {
                ...
                link.setAttribute('download', 'doctors_emails.csv');
                document.body.appendChild(link); // Required for FF
                link.click(); // This will download the data file named "doctors_emails.csv"
                document.body.removeChild(link); // Clean up the link element
                this.messageService.add({
                    severity: 'success',
                    summary: 'Success',
                    detail: 'Emails exported successfully',
                });
            },
            error: () => {
                this.messageService.add({
                    severity: 'error',
                    summary: 'Error',
                    detail: 'Failed to export emails',
                });
            },
        });
    }
